    ...
    if (grouping_stat == 'mean') {
      ...
      for (i in 1:(n - 1)) {
        for (j in (i + 1):n) {
          a <- means[[j]]
          b <- means[[i]]
          hellinger_dist <- (1 / sqrt(2)) * sqrt(sum((sqrt(a) - sqrt(b))^2))
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      plot(hc6, main = " ")
      plot(hc7, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with Hellinger distance"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
