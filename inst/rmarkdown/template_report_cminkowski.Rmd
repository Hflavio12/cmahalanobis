---
title: "Report Minkowski Distance"
    ...
    if (grouping_stat == 'mean') {
      ...
      for (i in 1:n) {
        for (j in i:n) {
          ...
          dissimilarity <- sum(abs(vec_i - vec_j)^(p))^(1/p)
      ...
      for (i in 1:n) {
        for (j in 1:n) {
          ...
          dissimilarity <- sum(abs(median_i - median_j)^(p))^(1/p)
      ...
      for (i in 1:n) {
        for (j in 1:n) {
          ...
          dissimilarity <- sum(abs(sds_i - sds_j)^(p))^(1/p)
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      plot(hc6, main = " ")
      plot(hc7, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with Minkowski distance"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
