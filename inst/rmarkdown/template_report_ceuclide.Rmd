---
title: "Report Euclidean Distance"
    ...
    if (grouping_stat == 'mean') {
      ...
      for (i in 1:n) {
        for (j in i:n) {
          ...
          dissimilarity <- sqrt(sum(vec_i - vec_j)^2)
      ...
      for (i in 1:n) {
        for (j in 1:n) {
          ...
          dissimilarity <- sqrt(sum(median_i - median_j)^2)  
      ...
      for (i in 1:n) {
        for (j in 1:n) {
          ...
          dissimilarity <- sqrt(sum(sds_i - sds_j)^2)
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      plot(hc6, main = " ")
      plot(hc7, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with euclidean distance"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
