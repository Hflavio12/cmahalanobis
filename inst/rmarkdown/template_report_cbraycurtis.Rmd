---
title: "Report Bray-Curtis Dissimilarity"
...
if (!is.null(distances) && !all(is.na(distances))) {
  print(distances, caption = "Bray-Curtis Dissimilarity")
} else {
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      plot(hc6, main = " ")
      plot(hc7, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with Bray-Curtis dissimilarity"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
