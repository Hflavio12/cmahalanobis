    ...
    if (grouping_stat == 'mean') {
      ...
      for (i in 1:n) {
        for (j in 1:n) {
          ...
          dice_similarity <- (2 * sum(pmin(specie_1, specie_2))) / (sum(specie_1) + sum(specie_2))
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      par(mfrow = c(3,3))
      hc <- hclust(dist_obj, method = 'median')
      hc1 <- hclust(dist_obj, method = 'centroid')
      plot(hc, main = " ")
      plot(hc1, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with Sorensen-Dice distance"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
