...
chamming <- function(dataset, formula, plot = TRUE, 
  ...
  if ("index" %in% grouping_vars) {
    ...
    h_dist <- Vectorize(function(i, j) sum(data_for_dist[i, ] != data_for_dist[j, ]))
...
if (!is.null(distances) && length(distances) > 0) {
  ...
  for (grouping_var in names(distances)) {
    ...
    if(attr(dist_obj, "Size") > 2) {
      ...
      plot(hc6, main = " ")
      plot(hc7, main = " ")
      mtext(paste("Dendrograms of groups in", grouping_var, "with various agglomeration methods, with Hamming distance"), side = 3, line = -2, outer = TRUE, cex = 1, font = 2)
      par(mfrow = c(1,1))
